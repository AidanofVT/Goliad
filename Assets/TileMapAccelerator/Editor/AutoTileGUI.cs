...
namespace TileMapAccelerator.Scripts
{
    public class AutoTileGUI : EditorWindow
    {
        ...
        [MenuItem("Window/Tilemap Accelerator/AutoTileset Generator")]
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Build Tileset to Resources"))
            {

                currentSaveFolder = Application.dataPath + "/TileMapAccelerator/Resources/";
                ...
                for (byte i = 0; i < AutoTileUtilities.TypeCount; i++)
                {
                    ...
                    AutoTileUtilities.latestAutoTilesetPrefix = currentPrefix;
                    AutoTileUtilities.latestAutoTilesetTexturePaths[i] = currentPrefix + i + ".png";

                    File.WriteAllBytes(currentSaveFolder + AutoTileUtilities.latestAutoTilesetTexturePaths[i], ImageConversion.EncodeToPNG(tile));
                }
